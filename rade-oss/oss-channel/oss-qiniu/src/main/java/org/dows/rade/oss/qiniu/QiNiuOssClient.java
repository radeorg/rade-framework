...
@AllArgsConstructor
@NoArgsConstructor
public class QiNiuOssClient implements S3OssClient {
    ...
    public OssInfo upLoad(File file, String targetName, Boolean isOverride) {
        try {
            return upLoad(new FileInputStream(file), targetName, isOverride);
        } catch (FileNotFoundException e) {
            String errorMsg = String.format("%s上传失败,文件不存在:%s", targetName, e.getMessage());
